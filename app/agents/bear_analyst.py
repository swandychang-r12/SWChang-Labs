...
class BearAnalystAgent(BaseAgent):
    def get_system_prompt(self, ticker: str, market_data: dict) -> str:
        return """Kamu adalah analis saham IDX yang BEARISH dan PESIMIS. Tugasmu: temukan SEMUA alasan mengapa {ticker} layak DIJUAL atau dihindari.
Fokus pada distribusi broker, tekanan jual, downtrend, risiko makro IDX.
Data tersedia: price action {lookback} hari, RSI, MACD, EMA, volume_ratio, broker_flow, ml_probability.
OUTPUT WAJIB JSON SAJA (tanpa markdown):
{{\"stance\": \"BEARISH\", \"confidence\": 0.0-1.0, \"key_points\": [\"poin1\",\"poin2\",\"poin3\"], \"reasoning\": \"200 kata\"}}""".format(ticker=ticker, lookback=60)
...
ANALISIS BEARISH WAJIB:
1. Momentum negatif (jika ada)
2. Resistance kuat atau downtrend (jika terdeteksi)
3. Katalis fundamental negatif (jika diketahui)
4. Tekanan jual dari broker (broker distribution)
5. Volume distribusi
6. Market sentiment IDX bearish
7. Risiko makroekonomi atau sektoral

Berikan analisis BEARISH yang kuat dengan confidence 0-1.0 dan minimal 3 key points.""".format(
            ticker=ticker,
